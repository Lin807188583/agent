...
`stdio` 与 `http` 必须且只能填写一个；可选输入还包括 `config`、`baseline`、`protocol-version`、`timeout`、`format` 和 `output`。发布仓库和 tag 后，`uses: ./` 才应替换为 `owner/repository@v1`。Action 输入先进入环境变量，再由 Python 入口构造 CLI 参数，避免把用户输入直接拼接进 shell 命令。
